...
#   C l a s s i c a l - S a f e t y - S e n t i n e l  
 
## 2026 Update Notes
...
- Use `python main.py` from project root for backend startup.

### Faster Demo Processing
- In UI Upload tab, use `Quick Mode` to process first 900 frames for faster live demos.
- The pipeline now uses single-pass inference for annotation (faster than previous double-pass flow).

### Tuned Risk Thresholds
- SAFE: risk < 0.32
- WARNING: 0.32 <= risk < 0.58
- CRITICAL: risk >= 0.58

### Faculty Demo Tips (Show RED Signal)
1. Keep backend running and confirm `/health` works.
2. Upload clips from `TU-DAT/Rash-Driving/` (start with `v5.mp4`, `v8.mp4`, `v7.mp4`).
3. Keep `Quick Mode` enabled for rapid iteration.
4. If everything appears SAFE, check backend logs to ensure YOLO loaded (not mock fallback).
